...
import { extractedRouteParams, getActivatedChild } from '../../../shared/router.utils';
...
export class ChecklistOverviewComponent implements OnInit {
  ...
  constructor(private store: Store<SkillsState>, private router: Router, private route: ActivatedRoute) {}
    ...
    this.route.params
      .pipe(
        ...
        tap(([entities, categories, editMode]) => {
          const { category } = extractedRouteParams(this.route.snapshot, 1);
