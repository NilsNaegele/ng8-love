...
import { calculatePercentage, computeScore, createChecklistItem, filterItems } from '../../state/skills-state.utils';
...
export namespace ChecklistSelectors {
  export const getFavoritesFilter = createSelector(
    SkillsSelectors.getChecklistState,
  ...
  export const getAllCategories = createSelector(
    SkillsSelectors.getCategoryEntities,
    SkillsSelectors.getItemEntities,
    ProjectsSelectors.getDisabledCategories,
    getScores,
  ...
  export const getSelectedCategory = createSelector(
    ...
    (routerState, categoryEntities, disabledCategories, scores): CategoryEntity => {
      const { category } = extractedRouteParams(routerState.root, 4);
  ...
  export const getSelectedItem = createSelector(
    ...
    (selectedCategory, items, routerState, projectItems, favorites): ChecklistItem => {
      const { item: id } = extractedRouteParams(routerState.root, 4);
